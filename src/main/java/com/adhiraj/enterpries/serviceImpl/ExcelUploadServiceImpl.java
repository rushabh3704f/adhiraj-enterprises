...
public class ExcelUploadServiceImpl implements ExcelUploadService{
	...
	@Autowired
	private BookingEntryRepository allocationDetailsRepository;
	...
	public void saveExcelData(MultipartFile file) {
		...
		try (InputStream is = file.getInputStream()) {
			...
			List<BookingEntryModel> allocationDetailsModelList=new ArrayList<>();
			for (int i = 1; i <= sheet.getLastRowNum(); i++) { // skip header row
				Row row = sheet.getRow(i);
				if (row == null) {
					continue;
				}else {
					BookingEntryModel entry = new BookingEntryModel();
					entry.setSrNo(getCellValue(row, 0));
					entry.setStockTrNum(getCellValue(row, 1));
					entry.setStockTrDate(getCellValue(row, 2));
					entry.setDeliveryNum(getCellValue(row, 3));
					entry.setVehicleChassisNum(getCellValue(row, 4));
					entry.setVehicleEngineNum(getCellValue(row, 5));
					...
					allocationDetailsModelList.add(entry);
				}
			}
			allocationDetailsRepository.saveAll(allocationDetailsModelList);
