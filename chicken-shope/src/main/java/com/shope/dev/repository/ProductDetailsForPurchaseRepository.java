...
public interface ProductDetailsForPurchaseRepository extends JpaRepository<ProductDetailsForPurchase, Long> {
	...
	@Query(value = "SELECT * FROM product_details_for_purchase m WHERE m.billing_number = :billing_number and is_active ='A'", nativeQuery = true)
	public List<ProductDetailsForPurchase> getAllProductDetailsForPurchaseList(@Param("billing_number") String billing_number);

	
	@Modifying
	@Transactional
	@Query(value = "UPDATE product_details_for_purchase set is_active ='D' where id=:id", nativeQuery = true)
	public void deleteProductDetailsForPurchaseById(@Param("id") String id);
